...
CHAPTER_PATTERNS = [r"^第\s*[\d零一二三四五六七八九十百千]+\s*[章回卷部节].*$", r"^Chapter\s+\d+.*$", r"^CHAPTER\s+\d+.*$", r"^\d+\.\s+.+$", r"^##\s+.+$"]
...
def parse_text(raw: bytes | str, filename: str) -> DistillationInput:
    ...
    for line in lines:
        stripped = line.strip()
        if any(re.match(pattern, stripped, re.I) for pattern in CHAPTER_PATTERNS): matches.append((offset, stripped.lstrip("# ")))
        offset += len(line)
